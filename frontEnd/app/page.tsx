...
export default function AlienPetZoo() {
  ...
  const [error, setError] = useState("")   // 🔹 Afegit l'state per errors
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError("") // 🔹 ara no peta perquè tenim l'state
    ...
    try {
      if (isRegistering) {
        ...
        if (!res.ok) throw new Error("Error al registrar-se")

        alert("Usuari registrat correctament! Pots fer login.")
        ...
        const res = await fetch("http://localhost:8080/auth/login", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            username: formData.email, // ⚠️ decideix si vols usar email o username
            password: formData.password,
          }),
        })
